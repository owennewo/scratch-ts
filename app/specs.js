...
System.register([], function(exports_1) {
    ...
    return {
        ...
        execute: function() {
            ...
            import Resources = assets.Resources;
        */
            // import {Bitmap} from './stubs/Bitmap';
            // import {Resources} from './assets/Resources';
            Specs = (function () {
                function Specs() {
                }
                // public static blockColor(categoryID:number):number {
                // 	if (categoryID > 100) categoryID -= 100;
                // 	for (var entry of Specs.categories) {
                // 		if (entry[0] == categoryID) return entry[2];
                // 	}
                // 	return Specs.undefinedColor;
                // }
                // public static entryForCategory(categoryName:string):any[] {
                // 	for (var entry of Specs.categories) {
                // 		if (entry[1] == categoryName) return entry;
                // 	}
                // 	return [1, categoryName, 0xFF0000]; // should not happen
                // }
                // public static nameForCategory(categoryID:number):string {
                // 	if (categoryID > 100) categoryID -= 100;
                // 	for (var entry of Specs.categories) {
                ...
                Specs.GetVar = "readVariable";
                Specs.SetVar = "setVar:to:";
                Specs.ChangeVar = "changeVar:by:";
                Specs.GetList = "contentsOfList:";
                Specs.Call = "call";
                Specs.ProcedureDef = "procDef";
                Specs.GetParam = "getParam";
                Specs.MotionCategory = 1;
                Specs.LooksCategory = 2;
                Specs.EventsCategory = 5;
                Specs.ControlCategory = 6;
                Specs.OperatorsCategory = 8;
                Specs.DataCategory = 9;
                Specs.MyBlocksCategory = 10;
                Specs.ListCategory = 12;
                Specs.ExtensionsCategory = 20;
                Specs.VariableColor = "#EE7D16"; // Scratch 1.4: 0xF3761D
                Specs.ListColor = "#CC5B22"; // Scratch 1.4: 0xD94D11
                Specs.ProcedureColor = "#632D99"; // 0x531E99;
                Specs.ParameterColor = "#5947B1";
                Specs.ExtensionsColor = "#4B4A60"; // 0x72228C; // 0x672D79;
                Specs.undefinedColor = 0xD42828;
                Specs.categories = [
                    ...
                    [9, "Data", Specs.VariableColor],
                    [10, "More Blocks", Specs.ProcedureColor],
                    [11, "Parameter", Specs.ParameterColor],
                    [12, "List", Specs.ListColor],
                    [20, "Extension", Specs.ExtensionsColor],
                ];
                Specs.commands = [
                    ...
                    // variables
                    ["set %m.var to %s", " ", 9, Specs.SetVar],
                    ["change %m.var by %n", " ", 9, Specs.ChangeVar],
                    ...
                    ["play drum %n for %n beats", " ", 98, "drum:duration:elapsed:from:", 1, 0.25],
                ...
                Specs.extensionSpecs = ["when %m.booleanSensor", "when %m.sensor %m.lessMore %n", "sensor %m.booleanSensor?", "%m.sensor sensor value", "turn %m.motor on for %n secs", "turn %m.motor on", "turn %m.motor off", "set %m.motor power to %n", "set %m.motor2 direction to %m.motorDirection", "when distance %m.lessMore %n", "when tilt %m.eNe %n", "distance", "tilt"];
                return Specs;
            })();
