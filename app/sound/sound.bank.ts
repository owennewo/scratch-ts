...
public static getNotePlayer(instNum, midiKey): NotePlayer {
    ...
    return new NotePlayer(Instr.samples[keyRange.sampleName], SoundBank.pitchForKey(keyRange.midiKey), keyRange.loopStart, keyRange.loopEnd, keyRange.envelope);
}
...
public static pitchForKey = function(midiKey: number): number {
    return 440 * Math.pow(2, (midiKey - 69) / 12); // midi key 69 is A=440 Hz
};

public static getDrumPlayer = function(drumNum: number, isMIDI: boolean, secs: number): NotePlayer {
