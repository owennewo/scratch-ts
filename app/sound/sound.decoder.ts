...
export class SoundDecoder {
    ...
    private indexTable = [-1, -1, -1, -1, 2, 4, 6, 8, -1, -1, -1, -1, 2, 4, 6, 8];
    ...
    constructor(wavFileData) {
        ...
        if (wavFileData != null) {
            let info = WAVFile.decode(wavFileData);
            this.info = info;
    ...
    interpolatedSample() {
        ...
        let out = this.fraction === 0 ? this.thisSample : this.thisSample + this.fraction * (this.nextSample - this.thisSample);
        return out / 32768.0;
    };

    // 16-bit samples, big-endian
    getSample16Uncompressed() {
        let result = 0;
